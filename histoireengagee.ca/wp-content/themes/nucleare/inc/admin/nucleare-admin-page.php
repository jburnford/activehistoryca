...
class Nucleare_Admin {
	...
	public function admin_menu() {
		...
		$page = add_theme_page( esc_html__( 'About', 'nucleare' ) . ' ' . $theme->display( 'Name' ), esc_html__( 'About', 'nucleare' ) . ' ' . $theme->display( 'Name' ), 'activate_plugins', 'nucleare-welcome', array( $this, 'welcome_screen' ) );
	...
	public static function hide_notices() {
		if ( isset( $_GET['nucleare-hide-notice'] ) && isset( $_GET['_nucleare_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_nucleare_notice_nonce'], 'nucleare_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'nucleare' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'nucleare' ) );
			}

			$hide_notice = sanitize_text_field( $_GET['nucleare-hide-notice'] );
	...
	public function welcome_notice() {
		...
		<div id="message" class="updated cresta-message">
			...
			<p><?php printf( esc_html__( 'Welcome! Thank you for choosing Nucleare! To fully take advantage of the best our theme can offer please make sure you visit our %swelcome page%s.', 'nucleare' ), '<a href="' . esc_url( admin_url( 'themes.php?page=nucleare-welcome' ) ) . '">', '</a>' ); ?></p>
			<p class="submit">
	...
	private function intro() {
		...
		<div class="cresta-theme-info">
			...
			<div class="welcome-description-wrap">
				...
				<p class="cresta-actions">
					<a href="<?php echo esc_url( 'https://crestaproject.com/downloads/nucleare/' ); ?>" class="button button-secondary" target="_blank"><?php esc_html_e( 'Theme Info', 'nucleare' ); ?></a>

					<a href="<?php echo esc_url( apply_filters( 'nucleare_pro_theme_url', 'http://crestaproject.com/demo/nucleare/' ) ); ?>" class="button button-secondary docs" target="_blank"><?php esc_html_e( 'View Demo', 'nucleare' ); ?></a>

					<a href="<?php echo esc_url( apply_filters( 'nucleare_pro_theme_url', 'http://crestaproject.com/demo/nucleare-pro/' ) ); ?>" class="button button-primary docs" target="_blank"><?php esc_html_e( 'View PRO version Demo', 'nucleare' ); ?></a>
		...
		<h2 class="nav-tab-wrapper">
			<a class="nav-tab <?php if ( empty( $_GET['tab'] ) && $_GET['page'] == 'nucleare-welcome' ) echo 'nav-tab-active'; ?>" href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'nucleare-welcome' ), 'themes.php' ) ) ); ?>">
			...
			<div class="changelog point-releases">
				<div class="under-the-hood two-col">
					<div class="col">
						...
						<p><a href="<?php echo admin_url( 'customize.php' ); ?>" class="button button-secondary"><?php esc_html_e( 'Customize', 'nucleare' ); ?></a></p>
					</div>
				...
				<tbody>
					...
					<tr>
						...
						<td class="btn-wrapper">
							<a href="<?php echo esc_url( apply_filters( 'nucleare_pro_theme_url', 'http://crestaproject.com/demo/nucleare-pro/' ) ); ?>" class="button button-secondary" target="_blank"><?php esc_html_e( 'View PRO version demo', 'nucleare' ); ?></a>
