...
function hoot_theme_customizer_options() {
	...
	$hoot_customizer_load_premiumsection = apply_filters( 'hoot_customizer_load_premiumsection', file_exists( $premium_features_file ) );
	if ( $hoot_customizer_load_premiumsection ) :
		...
		$pcontent .= '<div id="hoot-psection" class="hoot-psection"><div class="hoot-psection-content">';
			if ( !empty( $hoot_options_premium ) && is_array( $hoot_options_premium ) ):
				foreach ( $hoot_options_premium as $pkey => $pfeature ) :
					if ( !empty( $pfeature['type'] ) && ( $pfeature['type'] == 'bottom' || $pfeature['type'] == 'top' ) ) {
						$pcontent .= '<div class="pcontent-actions pcontent-' . $pfeature['type'] . '">';
						if ( !empty( $plink ) ) $pcontent .= '<a class="button button-primary" href="' . $plink . '" target="_blank"><i class="fa fa-star"></i> ' . __( 'Buy Now', 'responsive-brix' ) . '</a>';
						if ( !empty( $pdemo ) ) $pcontent .= '<a class="button button-secondary" href="' . $pdemo . '" target="_blank"><i class="fa fa-eye"></i> ' . __( 'Theme Demo', 'responsive-brix' ) . '</a>';
						$pcontent .= '</div>';
						if ( $pfeature['type'] == 'top' ) $intro = '';
					} else {
						$pcontent .= "<div class='section-premium-info{$intro}'>";
